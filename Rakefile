...
file GH_PAGES_REF => BUILD_DIR do
  cd PROJECT_ROOT do
    DOMAINS.each { |origin, _| repos[origin] = `git config --get remote.#{origin}.url`.strip }
  end
...
task :not_dirty do
  ...
  raise 'Directory not clean' unless ENV['ALLOW_DIRTY'] || /nothing to commit/ =~ `git status`
end
